...
function plot_data()
    fig = Figure(figure_padding=(1, 10, 1, 1), resolution=(JamesPlots.COLUMN_WIDTH, 2*JamesPlots.RESOLUTION[2]), font=projectdir("fonts", "MinionPro-Capt.otf"))
    ...
    data = select_data(results; dt=500.0, tmax=75.0, trajectories=1e3, bath_type=:ShenviGaussLegendre, initial_state=:Adiabatic)
    plot_data!(data, "Gauss-Legendre", ax1, ax2, COLORS[1], "short_gauss")
    data = select_data(results; dt=500.0, tmax=75.0, trajectories=1e3, bath_type=:TrapezoidalRule, initial_state=:Adiabatic)
    plot_data!(data, "Trapezoid", ax1, ax2, COLORS[3], "short_trapezoid")

    data = select_data(results; dt=500.0, tmax=10000.0, trajectories=1e3, bath_type=:ShenviGaussLegendre, initial_state=:Adiabatic)
    plot_data!(data, "NQCDynamics.jl", ax3, ax4, COLORS[1], "long_gauss")
    data = select_data(results; dt=500.0, tmax=10000.0, trajectories=1e3, bath_type=:TrapezoidalRule, initial_state=:Adiabatic)
